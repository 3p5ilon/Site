...
export default function NowReading() {
  ...
  return (
    <div className="block w-full max-w-3xl rounded-2xl overflow-hidden relative">
      <div className="bg-[#F1F1F1] dark:bg-[#1F1F1F] transition-colors rounded-2xl min-h-[7rem] sm:min-h-[7.75rem]">
        {loading || !book ? (
          ...
          >
            <div className="relative flex items-start w-full p-2 sm:p-2.5 min-h-[7rem] sm:min-h-[7.75rem]">
              ...
              <div className="flex-1 min-w-0 pr-8 sm:pr-10 flex flex-col justify-start self-start mt-1 sm:mt-1.5">
                <div className="text-sm sm:text-base font-semibold leading-4.5 sm:leading-5.5 break-words hyphens-auto">
                  {book.title}
                </div>
                {derivedAuthor && (
                  <div className="text-xs sm:text-sm font-medium text-neutral-600 dark:text-neutral-400 mt-1 sm:mt-1.5 break-words">
                    by {derivedAuthor}
                  </div>
                )}
              </div>
              <div className="absolute top-2.5 right-2.5 sm:top-3 sm:right-3 w-6 h-6 sm:w-7 sm:h-7 rounded-full bg-[#E9E2D7] flex items-center justify-center shadow-sm flex-shrink-0">
                <svg
                  role="img"
                  viewBox="0 0 24 24"
                  xmlns="http://www.w3.org/2000/svg"
                  className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-[#1E1914]"
                >
